...
class EDM():
    ...
    def _scale_batch(self, x, scale):
        return x * scale.view(-1, *([1] * (x.ndim - 1)))

    def _prepare_input(self, x):
        if getattr(self.cfg, "spatial_dims", 2) == 3 and x.ndim == 5:
    ...
    def model_forward_wrapper(self, x, sigma, t, use_ema=False, **kwargs):
        ...
        x_in, added_channel = self._prepare_input(x)
        if use_ema:
            model_output = self.ema(self._scale_batch(x_in, c_in), c_noise.view(-1, 1, 1).repeat(1, t.shape[1],  1), t)
        else:
            model_output = self.model(self._scale_batch(x_in, c_in), c_noise.view(-1, 1, 1).repeat(1, t.shape[1],  1), t)
        out = self._scale_batch(x_in, c_skip) + self._scale_batch(model_output, c_out)
        if added_channel:
            out = out.squeeze(2)
        return out
    ...
    def train_step(self, signals, t,  **kwargs):
        ...
        n = self._scale_batch(noise, sigma) # noise with sigma (scaling factor)
        D_yn = self.model_forward_wrapper(y + n, sigma, t)
        if self.cfg.gt_guide_type == 'l2':
            loss = self._scale_batch((D_yn - y) ** 2, weight)
        elif self.cfg.gt_guide_type == 'l1':
            loss = self._scale_batch(torch.abs(D_yn - y), weight)
        else:
...
def create_model(config):
    if getattr(config, "spatial_dims", 2) == 3:
        if not config.img_size_3d:
            raise ValueError("img_size_3d is required for 3D mode.")
        unet = Spatial_temporal_UNet3D(
            ...
            dropout=0,
            img_resolution=tuple(config.img_size_3d),
            label_dim=0,
    ...
    if config.dataset == 'am' or "ssf":
        ...
        if getattr(config, "spatial_dims", 2) == 3:
            if config.channels != 1:
                logger.warning("3D mode expects channels=1; overriding channels to 1.")
                config.channels = 1
            if not config.img_size_3d:
                config.img_size_3d = list(core.shape[2:])
            elif tuple(config.img_size_3d) != tuple(core.shape[2:]):
                raise ValueError(f"img_size_3d {config.img_size_3d} does not match core shape {core.shape[2:]}.")
    ...
    for step in range(config.num_steps):
        ...
        if config.save_signals_step and (step % config.save_signals_step == 0 or step == config.num_steps - 1):
            ...
            if config.spatial_dims == 3:
                sample_shape = [5, 24, config.channels, *config.img_size_3d]
            else:
