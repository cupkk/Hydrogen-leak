...
def standardize_train(x, sample_weight=None):
...
def fit_ridge_multioutput(x, y, alpha=1.0, sample_weight=None):
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
    if sample_weight is not None:
        ...
        sample_weight = np.maximum(sample_weight, 1e-12)
    x_std, x_mean, x_scale = standardize_train(x, sample_weight=sample_weight)
    y_std, y_mean, y_scale = standardize_train(y, sample_weight=sample_weight)

    if sample_weight is None:
        x_fit = x_std
        y_fit = y_std
    else:
        w_sqrt = np.sqrt(sample_weight)[:, None]
        x_fit = x_std * w_sqrt
        y_fit = y_std * w_sqrt

    xtx = x_fit.T @ x_fit
    eye = np.eye(xtx.shape[0], dtype=np.float64)
    beta = np.linalg.solve(xtx + float(alpha) * eye, x_fit.T @ y_fit)
