...
if __name__ == "__main__":
    ...
    parser.add_argument("--R", type=int, nargs=3, default=[1,48,48], help="size of Tucker core (R1 R2 R3)") 
    ...
    if config.save_last:
        ...
        scio.savemat(
            "./data/core_"+config.data_name+"_"+str(R[0])+"x"+str(R[1])+"x"+str(R[2])+"_"+str(formatted_time)+"_last.mat",
            {"core": tucker_core.detach().cpu().numpy()},
        )
        torch.save(
            basis_function,
            "./ckp/basis_"+config.data_name+"_"+str(R[0])+"x"+str(R[1])+"x"+str(R[2])+"_"+str(formatted_time)+"_last.pth",
        )
        print("save core/basis (last) successfully")
