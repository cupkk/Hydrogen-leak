...
#----------------------------------------------------------------------------
# 3D convolutional layer with optional up/downsampling.
...
class Conv3d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.up:
            x = torch.nn.functional.interpolate(x, scale_factor=2, mode='trilinear', align_corners=False)
        if self.down:
...
#----------------------------------------------------------------------------
# 3D U-Net block with optional up/downsampling and self-attention.
...
class Spatial_temporal_UNet3D(torch.nn.Module):
    def __init__(self,
        img_resolution,
        in_channels,
        out_channels,
        ...
        assert encoder_type in ['standard', 'skip', 'residual']
        assert decoder_type in ['standard', 'skip']
        if not (isinstance(img_resolution, (tuple, list)) and len(img_resolution) == 3):
            raise ValueError("img_resolution must be a 3-tuple for Spatial_temporal_UNet3D.")
        ...
        caux = in_channels
        for level, mult in enumerate(channel_mult):
            res_d = img_resolution[0] >> level
            res_h = img_resolution[1] >> level
            res_w = img_resolution[2] >> level
            res_tag = f'{res_d}x{res_h}x{res_w}'
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res_tag}_conv'] = Conv3d(in_channels=cin, out_channels=cout, kernel=3, **init)
                self.enc_temp[f'{res_tag}_conv'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
            else:
                self.enc[f'{res_tag}_down'] = UNetBlock3D(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                self.enc_temp[f'{res_tag}_down'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
                if encoder_type == 'skip':
                    self.enc[f'{res_tag}_aux_down'] = Conv3d(in_channels=caux, out_channels=caux, kernel=0, down=True, resample_filter=resample_filter)
                    self.enc[f'{res_tag}_aux_skip'] = Conv3d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res_tag}_aux_residual'] = Conv3d(in_channels=caux, out_channels=cout, kernel=3, down=True, resample_filter=resample_filter, fused_resample=True, **init)
                    caux = cout
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                attn = (res_d in attn_resolutions)
                self.enc[f'{res_tag}_block{idx}'] = UNetBlock3D(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
                self.enc_temp[f'{res_tag}_block{idx}'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            res_tag = f'{res_d}x{res_h}x{res_w}'
            if level == len(channel_mult) - 1:
                self.dec[f'{res_tag}_in0'] = UNetBlock3D(in_channels=cout, out_channels=cout, attention=True, **block_kwargs)
                self.dec[f'{res_tag}_in1'] = UNetBlock3D(in_channels=cout, out_channels=cout, **block_kwargs)
                self.dec_temp[f'{res_tag}_in0'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
                self.dec_temp[f'{res_tag}_in1'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
            else:
                self.dec[f'{res_tag}_up'] = UNetBlock3D(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
                self.dec_temp[f'{res_tag}_up'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
            ...
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res_d in attn_resolutions)
                self.dec[f'{res_tag}_block{idx}'] = UNetBlock3D(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
                self.dec_temp[f'{res_tag}_block{idx}'] = nn.Sequential(
                    nn.Conv1d(in_channels=cout, out_channels=num_temporal_latent*cout, kernel_size=3, stride=1, padding=1),
                    nn.ReLU(),
                    nn.Conv1d(in_channels=num_temporal_latent*cout, out_channels=cout, kernel_size=3, stride=1, padding=1),
                )
            ...
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res_tag}_aux_up'] = Conv3d(in_channels=out_channels, out_channels=out_channels, kernel=0, up=True, resample_filter=resample_filter)
                self.dec[f'{res_tag}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res_tag}_aux_conv'] = Conv3d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
