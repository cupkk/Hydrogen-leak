...
def cross_validate_alpha(x_train, y_train, groups, alpha_grid):
    rows = []
    for alpha in alpha_grid:
        fold_metrics = []
        for group, train_idx, val_idx in leave_one_group_out_indices(groups):
            model = fit_ridge_multioutput(x_train[train_idx], y_train[train_idx], alpha=alpha)
...
def main():
    ...
    for mode in modes:
        ...
        cv_rows = cross_validate_alpha(x_train, y_train, train_groups, alpha_grid)
        best_alpha = cv_rows[0]["alpha"]
        model = fit_ridge_multioutput(x_train, y_train, alpha=best_alpha)
